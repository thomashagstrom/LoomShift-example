...
Users can walk away from it the way they expect: a × in the top-right corner
closes it, and so does clicking outside the dialog or pressing Esc. The button
that opened it gets the keyboard's attention back every way out. A dialog that
must be answered can switch all three off.
